...
#### Post-release fixes ✅ (Faz A4 + B3/B4 — 2026-05-10)
...
- [x] **B3/B4 — Tray focus recovery (frame stream donması)**: `AppContext.tsx` window focus event handler'ında `markAppActivity()` çağrısı eklendi. `markAppActivity` artık `isScreenSleepingRef.current === true` iken: `setStreamingEnabled(true)` + `RETURN_TO_IDLE` firmware komutu + ref reset yapıyor. Önceki davranış: Eren tray'den geri döndüğünde stream dondurulmuş kalıyordu, firmware uyandırılmıyordu, window açık görünüyordu ama OLED freeze'deydi. Yeni davranış: focus alınca otomatik wake.

#### Post-release fixes ✅ (Faz A3 — 2026-05-10)
- [x] **A3.1 — device_profile handler WS-open recovery**: `AppContext.tsx` `handleWSOpen` — startup auto-connect'te backend `device_profile` broadcast ederken WS client henüz bağlı değilse mesaj kayboluyordu. `getStatus()` HTTP recovery path'i (`device_line` mevcut) şimdi `enumDevicesDoneRef` cancel/set de yapıyor. Önceki davranış: `enumDevicesDoneRef.current` hiç `true` set edilmeden `APP_CONNECTED` gönderiliyordu → Phase 2 fallback timer 5s bekleyip cold-start yolundan tetikleniyordu. Yeni davranış: WS-open recovery'de de Phase 2 anında tetikleniyor. Cold-start fallback 5s → 8s (daha geniş handshake penceresi).
- [x] **A3.2 — dropLogAt init fix**: `useAnimationEngine.ts` pump — `dropLogAt = 0` ile başlıyordu; ilk drop'ta `Date.now() - 0` = milyarlarca saniye log'a basılıyordu. `dropLogAt = Date.now()` ile init edildi.
- [x] **A3.3 — Serial backpressure: send() + lock**: `serial_service.py` `send()` artık `send_and_read` ile aynı `asyncio.Lock()` altında çalışıyor. PING write'ı frame exchange lock'u tutarken interleave etmiyordu → mixed RX stream riski ortadan kalktı.
- [x] **A3.4 — Serial backpressure: exchange write error soft-fail**: `serial_service.py` `send_and_read` içindeki write exception handler — daha önce ilk write timeout'ta `self._serial = None` yapıp port'u kapatıyordu. Şimdi `_write_fail_count` backoff sistemini kullanıyor; port yalnızca `_WRITE_MAX_FAILS` (=5) ardışık hatadan sonra kapanıyor. Tek burst timeout → false-positive USB disconnect tetiklemiyordu.
- [x] **A3.5 — USB disconnect monitor false-positive guard**: `main.py` — monitor şimdi 3 ardışık başarısız probe/is_connected=False sonrası broadcast yapıyor (önceki: 1 probe = disconnect). Frame burst sırasındaki geçici write timeout → port kapanma → hemen disconnect broadcast zinciri kırıldı.
...
- [ ] **T9.5.x-followup** — Continuous mode multi-turn: post-beta'da Gemini Live multi-session support (per-turn yeni session + context preservation) ile re-introduce edilecek. Beta'da tek-turn enforced (T9.5.7).
